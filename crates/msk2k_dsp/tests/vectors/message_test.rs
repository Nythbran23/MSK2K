...
fn test_cq_encode_decode() {
    ...
    // Decode
    let decoded = Message::from_format1_bits(&codec, &info_bits, &addr_bits, true, None).unwrap();
...
fn test_cold_call_encode_decode() {
    ...
    // Decode
    let decoded = Message::from_format1_bits(&codec, &info_bits, &addr_bits, false, Some("DJ5HG")).unwrap();
    assert_eq!(decoded.from_call, "GW4WND");
    ...
    for msg in messages {
        match msg.format {
            1 => {
                ...
                let decoded =
                    Message::from_format1_bits(&codec, &info_bits, &addr_bits, is_general, Some("DJ5HG")).unwrap();
...
fn test_parity_validation() {
    ...
    let result = Message::from_format1_bits(&codec, &info_bits, &addr_bits, false, Some("DJ5HG"));
...
fn test_invalid_callsign_rejection() {
    ...
    let result = Message::from_format1_bits(&codec, &info_bits, &addr_bits, true, None);
